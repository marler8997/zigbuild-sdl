...
//    yes: []const u8,
//) void {
//    config_header.values.put(field_name, .{ .have_header = .{ .path = path, .no = no, .yes = yes } }) catch @panic("OOM");
//}
...
fn render_cmake(
    ...
    while (line_it.next()) |raw_line| : (line_index += 1) {
        ...
        const line = expand_variables_cmake(step, allocator, raw_line, values) catch |err| switch (err) {
...
fn renderValueC(output: *std.ArrayList(u8), name: []const u8, value: Value) !void {
    switch (value) {
        ...
        .have_function => |have| {
            std.debug.panic("TODO: implement renderValueC for have_function name='{s}' have.name='{s}'", .{name, have.name});
        },
    }
}
            ...
            '@' => blk: {
                if (std.mem.indexOfScalarPos(u8, contents, curr + 1, '@')) |close_pos| {
                    ...
                    const value = values.get(key) orelse {
                        step.addError(
                            "missing cmake configuration variable '{s}'",
                            .{ key },
                        ) catch @panic("OOM");
                        return error.StepErrorAdded;
                    };
                    ...
                    switch (value) {
                        ...
                        .have_function => |have| {
                            const s = if (try haveFunction(have.name)) have.yes else have.no;
                            try result.appendSlice(s);
                        },
                    }
...
fn haveHeader(target: std.Target, path: []const u8, context: HaveHeaderContext) !bool {
    _ = target;
    _ = context;
    // It should be possible to implement this
    std.debug.panic("TODO: implement haveHeader for '{s}'", .{path});
}
fn haveFunction(name: []const u8) !bool {
    std.debug.panic("TODO: implement haveFunction for '{s}'", .{name});
}
